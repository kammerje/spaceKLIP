...
def annotate_compass(ax, image, wcs, xf=0.9, yf=0.1, length_fraction=0.07, color='white', fontsize=12, ):
    ...
    ax.annotate('N', xy=(x, y), xytext=(xn, yn),
                ...
                bbox=dict(facecolor='#4B0082', alpha=0.7, edgecolor='none'),
...
def display_coron_image(filename, ax=None, 
                        vmin=None, vmax=None, a=0.0001, dq_only=False):
    """
    ...
    interval = v.ManualInterval(vmin, vmax) if vmin is not None and vmax is not None else v.ManualInterval(low, high)

    stretch = v.AsinhStretch(a=a)
    norm = v.ImageNormalize(image,
                            interval=interval,
                            stretch=stretch)

    # Ensure ax is provided or create a new one
    if ax is None:
    ...
    # Annotations 
    ax.text(0.01, 0.99, annotation_text, bbox=dict(facecolor='#4B0082', alpha=0.7, edgecolor='none'),
...
@plt.style.context('spaceKLIP.sk_style')
def display_image_comparisons(database, base_dirs, restrict_to=None, 
                              save_filename=None, vmin=None, 
                              vmax=None, a=0.0001, interact=False, dq_only=False,
                              subtract_first=False):
    """
    Compare images before and after processing using interactive sliders or static plots.
    
    Parameters
    ----------
    database : object
        Database object containing image file information.
    base_dirs : list of str
        List of base directory names to constrain the plot setups.
    restrict_to : str or None
        Restrict to specific keys in the database.
    save_filename : str or None
        Filename to save the plots as a PDF.
    vmin : float or None
    ...
    interact : bool
        Whether to use interactive sliders.
    dq_only : bool
        Whether to only display data quality (DQ) information.
    subtract_first : bool
    ...
    # Initialize lists for filenames, bad pixel counts, and first SCI frame
    image_files = {base_dir: [] for base_dir in base_dirs}
    bp_counts = {base_dir: [] for base_dir in base_dirs}
    first_sci_frames = {base_dir: None for base_dir in base_dirs}
    first_sci_filenames = {base_dir: None for base_dir in base_dirs}

    if save_filename:
        pdf = PdfPages(save_filename)

    # Collect filenames for images and find the first SCI frame
    for key in database.obs.keys():
        if restrict_to and restrict_to not in key:
            continue
        ...
        for typestr in ['SCI', 'REF']:
            filenames = obstable[obstable['TYPE'] == typestr]['FITSFILE']
            for fn in filenames:
                root_dir = fn.split(os.sep)[0]
                base_fn = os.path.basename(fn)
                
                for base_dir in base_dirs:
                    fn_path = os.path.join(root_dir, base_dir, base_fn)
                    if os.path.exists(fn_path):
                        image_files[base_dir].append(fn_path)
                        dq = fits.getdata(fn_path, extname='DQ')
                        num_bad_pixels = np.sum((dq & 1) == 1)
                        bp_counts[base_dir].append(num_bad_pixels)
                        if subtract_first and first_sci_frames[base_dir] is None and typestr == 'SCI':
                            first_sci_frames[base_dir] = fits.getdata(fn_path)
                            first_sci_filenames[base_dir] = fn_path

                    else:
                        print(f"File does not exist: {fn_path}")
    ...
    def update_images(index):
        ...
        if num_dirs == 2:
            ...
            num_rows = 2
            num_cols = (num_dirs + 1) // 2
        else:
            num_rows, num_cols = 1, num_dirs

        fig, axes = plt.subplots(num_rows, num_cols, figsize=(num_cols * 10, num_rows * 10))
        axes = axes.flatten() if isinstance(axes, np.ndarray) else [axes]

        # Overall figure title using the first base directory filename
        first_base_dir = base_dirs[0]
        if image_files[first_base_dir]:
            fn = image_files[first_base_dir][index]
            fig.suptitle(os.path.basename(fn), fontsize=16)
        else:
            print(f"No files found in directory: {first_base_dir}")

        for ax, base_dir in zip(axes, base_dirs):
            if image_files[base_dir]:
                fn = image_files[base_dir][index]
                ax.clear()
                if subtract_first and first_sci_frames[base_dir] is not None:
                    if fn == first_sci_filenames[base_dir]:
                        ax.remove()  # Hide the axis for self-subtraction
                        continue
                    fig.suptitle(f'{os.path.basename(first_sci_filenames[base_dir])} - {os.path.basename(fn)}', fontsize=16)
                    # Copy the original file to a temporary file
                    suffix = '_' + fn.split('_')[-1]
                    with tempfile.NamedTemporaryFile(suffix=suffix, delete=False) as tmp:
                        temp_filename = tmp.name
                        shutil.copy2(fn, temp_filename)
                    
                    # Modify the SCI data in the temporary file
                    with fits.open(temp_filename, mode='update') as hdul:
                        sci_data = hdul['SCI'].data.astype(np.float32)
                        first_frame = first_sci_frames[base_dir].astype(np.float32)
                        if first_frame.ndim == 3:
                            first_frame = first_frame[0]
                        for i in range(sci_data.shape[0]):
                            sci_data[i] -= first_frame
                        hdul['SCI'].data = sci_data
                        hdul.flush()
                    
                    display_coron_image(temp_filename, ax=ax, vmin=vmin, vmax=vmax, a=a, dq_only=dq_only)
                    
                    # Manually delete the temporary file after use
                    os.remove(temp_filename)
                    # Add dashed white lines through the center
                    ny, nx = sci_data.shape[-2:]  # Handle both 2D and 3D arrays
                    cx, cy = nx // 2, ny // 2
                    ax.axhline(cy, color='white', linestyle='--', linewidth=1)
                    ax.axvline(cx, color='white', linestyle='--', linewidth=1)
                
                elif not subtract_first:
                    display_coron_image(fn, ax=ax, vmin=vmin, vmax=vmax, a=a, dq_only=dq_only)

                
                ax.set_title(base_dir)
                ax.legend(handles=[patches.Patch(color='orange', label=f'DO_NOT_USE = {bp_counts[base_dir][index]} px')], 
                          loc='lower center', bbox_to_anchor=(0.5, -0.18))
            else:
                print(f"No files found in directory: {base_dir}")
        
        # Hide any unused subplots
        for ax in axes[len(base_dirs):]:
            ax.axis('off')

        plt.tight_layout(rect=[0, 0, 1, 0.95])  # Adjust layout to fit the suptitle
        plt.show()
        
        return fig  # Return the figure object to be used for saving in PDF

    if interact:
        slider = widgets.IntSlider(value=0, min=0, max=len(next(iter(image_files.values()))) - 1, step=1, description='Image Index:')
        out = widgets.interactive_output(update_images, {'index': slider})
        display(slider, out)
    else:
        for i in range(len(next(iter(image_files.values())))):
            fig = update_images(i)
            if save_filename:
                pdf.savefig(fig)  # Save the current figure
                plt.close(fig)  # Close the figure to avoid overplotting
        
        if save_filename:
            pdf.close()

# Example usage:
# base_dirs = ['sigclip', 'other_dir', 'another_dir']
# display_image_comparisons(database, base_dirs, restrict_to=None, save_filename='comparison.pdf', vmin=0, vmax=100, a=0.0001, interact=False, dq_only=False, subtract_first=True)
