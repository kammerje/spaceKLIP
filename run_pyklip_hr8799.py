...
sys.path.append('/Users/jkammerer/Documents/Code/pyklip')
...
idir = '../HR8799/jwst_s1s2_data_prep/' # input directory with stage 2 calibrated data
odir = '../HR8799/pyklip_data/' # output directory for the plots and pyKLIP products
transmissiondir = '../transmissions/' # directory containing the transmission functions from https://jwst-docs.stsci.edu/jwst-near-infrared-camera/nircam-instrumentation/nircam-coronagraphic-occulting-masks-and-lyot-stops
...
mstar = {'F356W': float(np.load('../HR8799/pynrc_figs/seq_000_filt_F356W_starmag.npy')), # vegamag
         'F444W': float(np.load('../HR8799/pynrc_figs/seq_000_filt_F444W_starmag.npy')), # vegamag
         'F250M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F250M_starmag.npy')), # vegamag
         'F300M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F300M_starmag.npy')), # vegamag
         'F335M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F335M_starmag.npy')), # vegamag
         'F410M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F410M_starmag.npy')), # vegamag
         'F430M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F430M_starmag.npy')), # vegamag
         'F460M': float(np.load('../HR8799/pynrc_figs/seq_001_filt_F460M_starmag.npy')), # vegamag
         }
...
pa_ranges_bar = [(88.5-15., 88.5+15.), (268.5-15., 268.5+15.)] # deg; list of tuples defining the pizza slices that shall be considered when computing the contrast curves for the bar masks
...
pas_inject_rnd = [45., 135., 225., 315.] # deg; list of position angles at which fake planets shall be injected to compute the calibrated contrast curve for the round masks
...
class processor():
    ...
    def __init__(self,
        ...
        self.psfmask = {'F356W_MASKA430R_SUB320A430R': 'jwst_nircam_psfmask_0065',
                    ...
                    for i, key in enumerate(self.obs.keys()):
                        ...
                        for j in range(len(ra_off)):
                            ...
                            if (mode == 'RDI'):
                                ...
                                #     # test += [inttm*rotate(np.load('../HR8799/pynrc_figs/seq_001_filt_'+filt+'_psfs_%+04.0fdeg.npy' % self.obs[key]['PA_V3'][ww_sci[ii]])[j], -self.obs[key]['PA_V3'][ww_sci[ii]], reshape=False, mode='constant', cval=0.)]
                                #     test += [inttm*rotate(np.load('../HR8799/pynrc_figs/seq_000_filt_'+filt+'_psfs_%+04.0fdeg.npy' % self.obs[key]['PA_V3'][ww_sci[ii]])[j], -self.obs[key]['PA_V3'][ww_sci[ii]], reshape=False, mode='constant', cval=0.)]
                                #     totet += inttm # s
