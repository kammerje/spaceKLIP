...
def extract_companions(params, obs, overwrite=False):
    """
    ...
    for mode in params.mode:
        for annuli in params.annuli:
            for subsections in params.subsections:
                ...
                odir = params.odir+mode+'_annu%.0f_subs%.0f/FLUX/' % (annuli, subsections)
                ...
                for i, key in enumerate(obs.keys()):
                    ...
                    # over the rolls.
                    utils.get_transmission(params, obs, pxsc, filt, mask, subarr, odir, key)
                    ...
                    offsetpsf *= params.F0[filt]/10.**(params.mstar[filt]/2.5)/1e6/pxsc**2*(180./np.pi*3600.*1000.)**2 # MJy/sr
                    
                    # Loop through all companions.
                    res[key] = {}
                    for j in range(len(params.ra_off)):
                        
                        # Guesses for the fit parameters.
                        guess_dx = params.ra_off[j]/pxsc # pix
                        guess_dy = params.de_off[j]/pxsc # pix
                        ...
                        if ((overwrite == True) or ((not os.path.exists(fmdataset)) or (not os.path.exists(klipdataset)))):
                            ...
                            fm_class = fmpsf.FMPlanetPSF(inputs_shape=dataset.input.shape,
                                                         numbasis=np.array(params.truenumbasis[key]),
                                                         sep=guess_sep,
                                                         pa=guess_pa,
                                                         dflux=guess_flux,
                                                         ...
                                                         field_dependent_correction=partial(utils.correct_transmission, params=params))
                        ...
                        if (mode == 'RDI'):
                            ...
                            temp = utils.correct_transmission(temp, xx, yy, params)
                        ...
                        cc = plt.Circle((params.ra_off[j]/1000., params.de_off[j]/1000.), 10.*pxsc/1000., fill=False, edgecolor='green', linewidth=3)
                        ax[0].add_artist(cc)
                        ax[0].set_xlim([5., -5.])
                        ax[0].set_ylim([-5., 5.])
                        ...
                        if (params.verbose == True):
                            ...
                            print('   RA  = %.2f+/-%.2f mas (%.2f mas guess)' % (res[key][temp]['ra'], res[key][temp]['dra'], params.ra_off[j]))
                            print('   DEC = %.2f+/-%.2f mas (%.2f mas guess)' % (res[key][temp]['de'], res[key][temp]['dde'], params.de_off[j]))
                            try:
                                condir = params.idir[:params.idir.find('/')]+'/pynrc_figs/'
